        ...
        <Card class="overflow-hidden">
          <div style="height: 600px" class="relative z-0">
            <l-map ref="map" v-model:zoom="zoom" :center="mapCenter as [number, number]"
              ...
              <!-- Photo Markers -->
              <l-marker v-for="photo in photosWithCoordinates" :key="photo.id"
                        :lat-lng="[photo.latitude!, photo.longitude!]" @click="selectPhoto(photo)">
              ...
              <!-- Route Line -->
              <l-polyline v-if="routeCoordinates.length > 0" :lat-lngs="routeCoordinates as [number, number][]"
                          :weight="3"
                          color="#3B82F6" />
            </l-map>
          </div>
      ...
      <Dialog v-model:open="deleteDialogOpen">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete "{{ trip.title }}" and all {{ trip.photos.length }} photos. This action
              cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
      ...
      <Sheet v-model:open="shareSheetOpen">
        <SheetContent>
          <SheetHeader>
            ...
            <SheetDescription>
              Control who can access this trip
            </SheetDescription>
          </SheetHeader>
          ...
          <div class="py-6 space-y-6">
            ...
            <div class="flex items-center justify-between">
              <div class="space-y-0.5">
                ...
                <p class="text-sm text-muted-foreground">
                  {{ localIsPublic ? 'Anyone can view this trip' : 'Only people with the link can view' }}
                </p>
              </div>
              <Switch
                id="public-toggle"
            ...
            <div v-if="!localIsPublic" class="space-y-4">
              <div class="space-y-2">
                ...
                <p class="text-sm text-muted-foreground">
                  {{ shareLink ? 'Share this link with people you want to access the trip' : 'Generate a secure link to share this trip' }}
                </p>
              </div>
              ...
              <div v-if="shareLink" class="space-y-3">
                <div class="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    :disabled="isCopying"
                  >
                    <svg v-if="copySuccess" class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <Dialog v-model:open="regenerateDialogOpen">
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will create a new share link and invalidate the old one. Anyone with the previous link will no longer be able to access this trip.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
